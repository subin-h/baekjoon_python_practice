N=int(input())
first_line='어느 한 컴퓨터공학과 학생이 유명한 교수님을 찾아가 물었다.'
def factorial_iterative1(n):
    ...
    factorial_iterative1(n-1)
    

def factorail_iterative2(n):
    if n<0:
        return 0
    print('____'*n+'라고 답변하였지.')
    factorail_iterative2(n-1)

print(first_line)
factorial_iterative1(N)
print('____'*N+'"재귀함수가 뭔가요?"')
print('____'*N+'"재귀함수는 자기 자신을 호출하는 함수라네"')
factorail_iterative2(N)
